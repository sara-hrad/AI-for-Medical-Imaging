...
2.  Use the provided `dicom_2_nifti.py` script to perform the conversion.
    ...
    ```
    This script will process the DICOM files from `dicom_original` and save the resulting NIfTI files (e.g., `studyID.nii.gz`) into a directory named `input_vesselFM`.
...
2.  **Run Inference:**
    ...
    ```
    The script will use the settings defined in the modified `inference.yaml` to process the NIfTI files located in the `input_vesselFM` directory (or as specified in your `inference.yaml` if you further customized it). The segmented output masks (also in NIfTI format) will be saved to the output directory specified in the `inference.yaml` file.

## Step 4: Convert Segmented Masks (NIfTI to DICOM)

If you need to convert the NIfTI segmentation masks generated by VesselFM back into DICOM series perform following steps.
